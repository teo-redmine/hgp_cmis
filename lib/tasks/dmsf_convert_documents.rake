...
desc <<-END_DESC
Convert project documents to HGP_CMIS folder/file structure.

Converted project must have documents and may not have HGP_CMIS active!
...
Example:
  rake redmine:hgp_cmis_convert_documents project=test RAILS_ENV="production"
END_DESC
#require File.expand_path(File.dirname(__FILE__) + "/../../../../../config/environment")

class HgpCmisConvertDocuments
  
  def self.convert(options={})
    dry = options[:dry] ? options[:dry] == "true" : false
    replace = options[:invalid] ? options[:invalid] == "replace" : false
    
    projects = options[:project] ? [Project.find(options[:project])] : Project.all

    projects.reject! {|project| project.module_enabled?("hgp_cmis") }
    ...
    unless projects.nil? || projects.empty?
      projects.each do |project|
        ...
        project.enabled_module_names = (project.enabled_module_names << "hgp_cmis").uniq unless dry
        ...
        project.documents.each do |document|
          ...
          folder.user = (a = document.attachments.order(created_on: :asc).first ? a.author : User.first)
          ...
          document.attachments.each do |attachment|
            begin
              ...
              i = 1
              suffix = ""
              while files.index{|f| f.name == (HgpCmisFileRevision.remove_extension(file.name) + suffix + File.extname(file.name))}
                i+=1
                suffix = "_#{i}"
              end
              ...
              file.name = HgpCmisFileRevision.remove_extension(file.name) + suffix + File.extname(file.name)
              ...
              revision.title = HgpCmisFileRevision.filename_to_title(attachment.filename)
...
namespace :redmine do
  task :hgp_cmis_convert_documents => :environment do
    options = {}
    options[:project] = ENV['project'] if ENV['project']
    options[:dry] = ENV['dry'] if ENV['dry']
    options[:invalid] = ENV['invalid'] if ENV['invalid']

    HgpCmisConvertDocuments.convert(options)
  end
end
