...
module ActiveCMIS
  class Repository
    ...
    # Urls de acceso a CMIS (sin incluir la raíz de alfresco)
    @object_by_path_url = "/s/workspace:SpacesStore/arg/p?path={path}&filter={filter}&includeAllowableActions={includeAllowableActions}&includePolicyIds={includePolicyIds}&includeRelationships={includeRelationships}&includeACL={includeACL}&renditionFilter={renditionFilter}"
    ...
    # @return [Object]
    def object_by_id(id, parameters = {"renditionFilter" => "*", "includeAllowableActions" => "true", "includeACL" => false})
      parameters["filter"] = "cmis:creationDate,cm:owner,cmis:changeToken,cm:description,cmis:objectId,cmis:objectTypeId,cmis:lastModifiedBy,cmis:name,cmis:createdBy,cmis:baseTypeId,cmis:creationDate,cmis:lastModificationDate,cmis:contentStreamMimeType"
      ActiveCMIS::Object.from_parameters(self, parameters.merge("id" => id))
    end

    def object_by_path(path, parameters = {"renditionFilter" => "*", "includeAllowableActions" => "true", "includeACL" => false})
      parameters["filter"] = "cmis:creationDate,cm:owner,cmis:changeToken,cm:description,cmis:objectId,cmis:objectTypeId,cmis:lastModifiedBy,cmis:name,cmis:createdBy,cmis:baseTypeId,cmis:creationDate,cmis:lastModificationDate,cmis:contentStreamMimeType"
      ActiveCMIS::Object.path_from_parameters(self, parameters.merge("path" => path))
    end
