...
module ActiveCMIS
  module Internal
    class Connection
      ...
      def get(url)
        #url += "&filter=cmis%3AcreationDate%2Ccm%3Aowner%2Ccmis%3AchangeToken%2Ccm%3Adescription%2Ccmis%3AobjectId%2Ccmis%3AobjectTypeId%2Ccmis%3AlastModifiedBy%2Ccmis%3Aname%2Ccmis%3AcreatedBy%2Ccmis%3AbaseTypeId%2Ccmis%3AcreationDate%2Ccmis%3AlastModificationDate"
        uri = normalize_url(url)
        request_uri = uri.request_uri
        if !request_uri.include?"&filter="
          if !request_uri.include?"?"
            request_uri += "?"
          end
          request_uri += "&filter=cmis:creationDate,cm:owner,cmis:changeToken,cm:description,cmis:objectId,cmis:objectTypeId,cmis:lastModifiedBy,cmis:name,cmis:createdBy,cmis:baseTypeId,cmis:creationDate,cmis:lastModificationDate,cmis:contentStreamMimeType"
        end
      ...
      def get_xml(url)
        #cambio STRICT por DEFAULT_XML
        Nokogiri::XML.parse(get(url), nil, nil, Nokogiri::XML::ParseOptions::DEFAULT_XML)
      end
      ...
      def handle_request(uri, req)
          ...
          if 200 <= status && status < 300
            ...
            raise HTTPError::ServerError.new("The server encountered an internal error #{status} (this could be a client error though) (#{uri}):\n" + body)
          end
        end
      end
    end
