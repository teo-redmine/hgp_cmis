...
class HgpCmisZip
  ...
  def initialize()
    @zip = Tempfile.new(["hgp_cmis_zip",".zip"])
  ...
  def add_file(file, root_path = nil)
    string_path = file.folder.nil? ? "" : file.folder.hgp_cmis_path_str + "/"
    ...
    begin
      string_path = Iconv.conv(Setting.plugin_hgp_cmis["hgp_cmis_zip_encoding"], "utf-8", string_path)
    rescue
    end
  ...
  def add_folder(folder, root_path = nil)
    string_path = folder.hgp_cmis_path_str + "/"
    string_path = string_path[(root_path.length + 1) .. string_path.length] if root_path
    #TODO: somewhat ugly conversion problems handling bellow
    begin
      string_path = Iconv.conv(Setting.plugin_hgp_cmis["hgp_cmis_zip_encoding"], "utf-8", string_path)
