...
class HgpCmisFilesCopyController < ApplicationController
  unloadable
  ...
  def new
    ...
    @target_project ||= @project if User.current.allowed_to?(:hgp_cmis_file_manipulation, @project)
    if HgpCmisFile.allowed_target_projects_on_copy.blank?
      flash.now[:warning] = l(:warning_no_project_to_copy_file_to)
    else
      @target_project ||= HgpCmisFile.allowed_target_projects_on_copy[0]
    end
  ...
  def create
    ...
    redirect_to :controller => "hgp_cmis_files", :action => "show", :id => new_file
  end

  def move
    @target_project = HgpCmisFile.allowed_target_projects_on_copy.detect {|p| p.id.to_s == params[:target_project_id]} if params[:target_project_id]
    unless @target_project && User.current.allowed_to?(:hgp_cmis_file_manipulation, @target_project) && User.current.allowed_to?(:hgp_cmis_file_manipulation, @project)
      render_403
      return
    end
    ...
    redirect_to :controller => "hgp_cmis_files", :action => "show", :id => @file
  end
  ...
  def find_file
    ...
    @file = HgpCmisFile.find(params[:id])
